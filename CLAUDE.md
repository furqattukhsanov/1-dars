...
- 🔴 **BOT_TOKEN almashtirilsa WEBHOOK ham qayta ro'yxatdan o'tkazilsin** (2026-08-13). BotFather'da token revoke qilinganda Telegram webhook manzilini ham **o'chiradi**, ya'ni ro'yxat ikki emas, **uch qadam**: (1) `.env` ga yoz, (2) servisni ko'tar, (3) `setWebhook` + `getWebhookInfo` bilan TASDIQLA. `getMe` yashil bo'lishi YETARLI EMAS — u webhook haqida hech narsa demaydi.
  Sabab: o'sha kuni uchinchi qadam tushib qolgan va **saytga kirish butunlay o'lgan** — botdagi «Start» hech narsa qilmasdi, chunki Telegram xabarni oladi-yu uzatadigan manzil yo'q edi (`getWebhookInfo` → `url: ""`, 6 ta yangilanish navbatda qotgan). Nuqson JIMGINA keldi: sayt `/api/auth/web/start` ga 200 qaytarardi, server `/api/telegram-webhook` da 401 berib tirik turardi, token `getMe` da sog'lom edi — uzilgan joy faqat ORADA edi va uni hech qaysi ekran ko'rsatmasdi.
  Tiklash buyrug'i sirlarni serverning O'ZIDA o'qisin (qiymat yozishmaga chiqmasin):
  `T=$(grep -m1 "^BOT_TOKEN=" .env | cut -d= -f2-)` … `curl -X POST ".../setWebhook" -d "url=https://lolamarket.uz/api/telegram-webhook" -d "secret_token=$S"`.
