/* ====================================================
   LolaMarket Mini App — PWA
   ----------------------------------------------------
   1) Service worker'ni ro'yxatdan o'tkazadi (offline sahifa uchun)
   2) "Uy ekraniga qo'shish" taklifini ko'rsatadi
   Telegram ichida ochilganda taklif ko'rsatilmaydi — u yerda ilova
   allaqachon Telegram'ning o'zida turadi.
   ==================================================== */
(function () {
  'use strict';

  var DISMISS_KEY = 'lm_pwa_dismissed_at';
  var DISMISS_DAYS = 14;
  ...
  // service worker umuman ro'yxatdan o'tmaydi — 2026-07-30 da landing saytida
  // aynan shu bo'ldi. Bu yerda ham xuddi shu naqsh bor edi, oldini olamiz.
  ...
  if (inTelegram || isStandalone) return;
  ...
  function showBanner(sub, actionLabel, onAction) {
    ...
    // Pastki navigatsiya ustida tursin — uni to'sib qo'ymasin.
    // Nav balandligi safe-area padding'ini o'z ichiga oladi, shuning uchun
    // env() ni ustiga qo'shmaymiz.
    var nav = document.querySelector('.app-nav-wrap:not(.hidden)');
    var navH = nav ? nav.getBoundingClientRect().height : 0;
    if (navH > 0) banner.style.bottom = (navH + 8) + 'px';

    // Boshlang'ich holatni majburan hisoblatamiz — shundan keyin klass
    // qo'shilsa animatsiya ishlaydi. requestAnimationFrame'ga tayanmaymiz:
    // sahifa fonda bo'lsa u chaqirilmay qolishi va banner ko'rinmasligi mumkin.
